...
@section('fb_url')
    <meta property="og:url"				content="https://dooball-th.com/{{ $league_url }}/result">
@endsection
@section('fb_title')
...
@endsection
@section('global_url')
    <link href="https://dooball-th.com/{{ $league_url }}/result" rel="canonical">
@endsection
                    ...
                    <div class="card-body">
                        @if (count($tab_list) > 0)
                            <ul class="nav nav-tabs mt-1" id="myTab" role="tablist">
                                ...
                                @foreach($tab_list as $month => $text)
                                    <li class="nav-item month-list">
                                        <a class="nav-link {{ ($indexTab == (count($tab_list) - 1)) ? 'active' : '' }}" id="m{{ $month }}-tab" data-index="{{ $indexTab }}" data-toggle="tab" href="#m{{ $month }}" role="tab" aria-controls="m{{ $month }}" aria-selected="true">
                                            {{ $text }}
                                        </a>
                                    </li>
                                    @php
                            ...
                            <div class="tab-content">
                                ...
                                @endphp
                                @foreach($tab_list as $month => $text)
                                    <div class="tab-pane fade {{ ($indexContent == (count($tab_list) - 1)) ? 'show active' : '' }}" id="m{{ $month }}" role="tabpanel" aria-labelledby="m{{ $month }}-tab">
...
@section('custom-scripts')
    ...
    <script type="text/javascript" src="{{ asset('frontend/js/league-result/months.js?' . time()) }}"></script>
