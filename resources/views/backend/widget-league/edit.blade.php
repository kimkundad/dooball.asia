...
@section('content')
    @section('breadcrumb')
        ...
        <li><a onclick="Javascript:void(0);"><span class="icon icon-double-angle-right"></span>แก้ไขหัวข้อ</a></li>
    @endsection
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                <form role="form" id="widget_league_form">
                                    @csrf
                                    <input type="hidden" name="wgl_id" value="{{ $form->id }}" />

                                    <div class="form-group">
                                        <label for="widget_title" class="col-lg-2 col-md-2 col-sm-6 col-xs-12 form-label text-right">ชื่อหัวข้อ&nbsp;:</label>
                                        <div class="col-lg-8 col-md-8 col-sm-6 col-xs-12">
                                            <input type="text" class="form-control" name="widget_title" id="widget_title" value="{{ $form->widget_title }}" placeholder="ตัวอย่าง: พรีเมียร์ลีก อังกฤษ" maxlength="125" />
                                        </div>
                                    </div>
                                            ...
                                            <select class="form-control" name="league_id" id="league_id">
                                                ...
                                                @if ($leagues->count() > 0)
                                                    @foreach ($leagues->get() as $league)
                                                        <option value="{{ $league->id }}" {{ (($form->league_id == $league->id) ? 'selected' : '') }}>{{ $league->name_en }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-lg-8 col-md-8 col-sm-6 col-xs-12">
                                            <input type="text" class="form-control" name="page_url" id="page_url" value="{{ $form->page_url }}" placeholder="ตัวอย่าง: result" maxlength="100" />
                                        </div>
                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="form-group">
        ...
        function submitForm(this_form) {
            Swal.fire({
                ...
                onOpen: () => {
                    ...
                    $.ajax({
                        url: $('#base_url').val() + '/api/admin/league-decoration/save-update',
                        type: 'POST',
                        data: formData,
                        dataType: 'json',
                        cache: false,
