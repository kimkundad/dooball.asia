...
@section('content')
    @section('breadcrumb')
        ...
        <li><a onclick="Javascript:void(0);"><span class="icon icon-double-angle-right"></span>แก้ไขเพจ</a></li>
    @endsection
                            ...
                            <div class="tab-content">
                                <div id="thai" class="tab-pane fade in active">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="page_name" id="page_name" value="{{ $form->page_name }}" maxlength="200" />
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="page_condition" id="page_condition" onchange="checkShowPageCondition()">
                                            <option value="L" {{ (($form->page_condition == 'L') ? 'selected' : '') }}>ชื่อลีก</option>
                                            <option value="T" {{ (($form->page_condition == 'T') ? 'selected' : '') }}>ชื่อทีม</option>
                                        </select>
                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="form-group league">
                                        <label for="league_name" class="form-label">ชื่อลีก&nbsp;:</label>
                                        <input type="text" class="form-control" name="league_name" id="league_name" value="{{ $form->league_name }}" maxlength="125" />
                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="form-group team">
                                        <label for="team_name" class="form-label">ชื่อทีม&nbsp;:</label>
                                        <input type="text" class="form-control" name="team_name" id="team_name" value="{{ $form->team_name }}" maxlength="125" />
                                    </div>
                                    <div class="clearfix"></div>
                                            ...
                                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                                <div class="table-responsive">
                                                    <table class="table">
                                                        ...
                                                        <tbody id="tbody_page_key">
                                                            @if(count($form->keys) > 0)
                                                                @foreach($form->keys as $key => $value)
                                                                    <tr id="tr_page_key_{{ $key }}">
                                                                        <td>
                                                                            <input type="hidden" name="key_ids[]" id="key_ids_{{ $key }}" value="{{ $value->id }}">
                                                                            <select class="form-control" class="form-control" name="key_names[]" id="key_names_{{ $key }}" onchange="changeKeyName($(this))">
                                                                                <option value="">--- เลือกตัวแปร ---</option>
                                                                                @foreach($filters as $col)
                                                                                    <option value="{{ $col['value'] }}" {{ ($value->key_name == $col['value']) ? 'selected' : '' }}>{{ $col['name'] }}</option>
                                                                                @endforeach
                                                                            </select>
                                                                        </td>
                                                                        <td>
                                                                            <input type="text" class="form-control key-val" name="key_values[]" id="key_values_{{ $key }}" value="{{ $value->key_value }}" onkeyup="keyInput($(this))">
                                                                        </td>
                                                                        <td>
                                                                            <button type="button" class="btn btn-sm btn-danger" onclick="delKeyRow($(this))"><i class="fa fa-trash"></i></button>
                                                                        </td>
                                                                    </tr>
                                                                @endforeach
                                                            @endif
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" name="description_th" id="description_th" rows="5">{{ $form->description }}</textarea>
                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="clearfix"></div>
                                    <div class="form-group">
                                        <div class="admin-cbox mt-10">
                                            <input type="checkbox" class="chkb" name="show_on_menu" id="show_on_menu" value="1" {{ (((int) $form->show_on_menu == 1)? 'checked' : '') }} />
                                    ...
                                    <div class="clearfix"></div>
                                    <div class="form-group">
                                        <label for="seo_title_th" class="form-label">SEO Title&nbsp;:</label>
                                        <input type="text" class="form-control" name="seo_title_th" id="seo_title_th" value="{{ $form->seo_title }}" maxlength="200" />
                                    </div>
                                    <div class="clearfix"></div>
                                    <div class="form-group">
                                        <label for="seo_description_th" class="form-label">SEO Description&nbsp;:</label>
                                ...
                                <div id="english" class="tab-pane fade">
                                    <div class="form-group">
                                        <label for="title_en" class="form-label">หัวข้อ&nbsp;:</label>
                                    ...
                                    <div class="clearfix"></div>
                                    <div class="form-group">
                                        <label for="detail" class="form-label">รายละเอียดในส่วนอื่นๆ&nbsp;:</label>
        ...
        function submitForm(this_form) {
            Swal.fire({
                ...
                onOpen: () => {
                    ...
                    $.ajax({
                        url: $('#base_url').val() + '/api/admin/page/save-update',
                        type: 'POST',
                        data: formData,
                        dataType: 'json',
                        cache: false,
