...
@section('content')
    @section('breadcrumb')
        ...
        <li><a onclick="Javascript:void(0);"><span class="icon icon-double-angle-right"></span>แก้ไขออนเพจ</a></li>
    @endsection
    <form role="form" id="onpage_form" enctype="multipart/form-data">
        @csrf
        <input type="hidden" name="onpage_id" value="{{ $form->id }}">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="form-group">
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                            <input type="text" class="form-control" name="code_name" id="code_name" placeholder="ตัวอย่าง: home, game, livescore" value="{{ $form->code_name }}">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
                            <input type="text" class="form-control" name="seo_title" id="seo_title" placeholder="SEO Title" value="{{ $form->seo_title }}" maxlength="200">
                        </div>
                    </div>
                    <div class="clearfix"></div>
                    <div class="form-group">
                        ...
                        <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12">
                            <input type="text" class="form-control" name="seo_description" id="seo_description" placeholder="SEO Description" value="{{ $form->seo_description }}" maxlength="200">
                        </div>
                    </div>
                    <div class="clearfix"></div>
                    <div class="form-group">
                            ...
                            <select class="form-control" name="page_top" id="page_top">
                                ...
                                @if ($page_list->count() > 0)
                                    @foreach($page_list->get() as $page)
                                        @php
                                            $selected = ($page->id == $form->page_top) ? 'selected' : '';
                                        @endphp
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                            <select class="form-control" name="page_bottom" id="page_bottom">
                                ...
                                @if ($page_list->count() > 0)
                                    @foreach($page_list->get() as $page)
                                        @php
                                            $selectedBtt = ($page->id == $form->page_bottom) ? 'selected' : '';
                                        @endphp
                                        <option value="{{ $page->id }}" {{ $selectedBtt }}>{{ $page->page_name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
        ...
        function submitForm(this_form) {
            Swal.fire({
                ...
                onOpen: () => {
                    ...
                    $.ajax({
                        url: $('#base_url').val() + '/api/admin/on-page/save-update',
                        type: 'POST',
                        data: formData,
                        dataType: 'json',
                        cache: false,
